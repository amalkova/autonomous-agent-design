...
from specialist_runners import (
    run_estimation_plan_execute,
    run_solution_security_react,
)
...
def build_mas_graph(
    ...
    def input_guardrail_node(
        ...
        rate_limit = (
            active_rate_limiter.check_and_record(
                state.get(
                    "session_id",
                    "anonymous",
                )
        ...
        if not rate_limit.allowed:
            return {
                ...
                "agent_output": (
                    "Запит заблоковано rate limiter."
                ),
                "trajectory": [
                    log_event(
                        "rate_limiter",
...
async def build_production_mas(
    ...
    requirements_agent = create_agent(
        ...
        system_prompt=(
            ...
            "Працюй як Agentic RAG: спочатку шукай "
            "релевантні правила у persistent ChromaDB, "
            "потім перевіряй completeness, readiness, "
            "acceptance criteria та handover gaps через "
            "MCP tools. Не вигадуй відсутні дані."
        ),
    )
    ...
    if log_trajectory:
        selected_agent = str(
            ...
            or (
                ...
                else "demand_supervisor"
            )
        )
        final_answer = str(
            result.get("final_answer")
            or "MAS не сформувала відповідь."
        )
        ...
        save_trajectory(
            ...
            metadata={
                "thread_id": thread_id,
                "route_reasoning": result.get(
                    "route_reasoning",
                ),
                "handoff_count": result.get(
                    "handoff_count",
                    0,
                ),
                "trajectory_events": result.get(
                    "trajectory",
                    [],
                ),
            },

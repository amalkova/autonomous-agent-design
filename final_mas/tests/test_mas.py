...
@pytest.mark.asyncio
async def test_rate_limit_blocks_before_supervisor() -> None:
    """Rate limiter блокує повторний session request."""
    ...
    graph = build_mas_graph(
        ...
        agent_runners={
            "requirements_agent": runner,
            "solution_security_agent": runner,
            "estimation_agent": runner,
        },
    ...
    first = await run_mas_query(
        graph,
        "Перевір DEM-930.",
        "same-session",
    )
    second = await run_mas_query(
        graph,
        "Перевір DEM-931.",
        "same-session",
    )
